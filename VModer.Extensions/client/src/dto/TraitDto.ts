...
export function getFlags(value: TraitType): TraitType[] {
  ...
  if (hasFlag(value, TraitType.Land)) {result.push(TraitType.Land);}
  if (hasFlag(value, TraitType.Navy)) {result.push(TraitType.Navy);}
  if (hasFlag(value, TraitType.CorpsCommander)) {result.push(TraitType.CorpsCommander);}
  if (hasFlag(value, TraitType.FieldMarshal)) {result.push(TraitType.FieldMarshal);}
  if (hasFlag(value, TraitType.Operative)) {result.push(TraitType.Operative);}

  return result.length > 0 ? result : [TraitType.None];
}

export { FileOrigin, TraitType };
